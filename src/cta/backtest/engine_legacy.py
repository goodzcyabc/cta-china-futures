"""【冻结】2026-09-22 前的日频回测引擎,只用于复现旧报告数字(report_v0.3 及 design_log 一–十七 的回测);新研究一律用 engine.py。

与新引擎的差异:按 T+1 开盘价定手数;未成交顺延陈旧目标;换月三腿(等手数移仓 + 当日再平衡)且不过涨跌停闸门;
按面板级 settle/prev_settle 盯市。原文档:
