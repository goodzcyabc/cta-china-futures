...
### 并行账本(2026-09-18 起)
- `paper/`:v0.1(tsmom + carry),主账本;`paper_v03/`:v0.3(+ 仓单水平);`paper_v03p/`:v0.3 剔除 5 个 IS 亏钱品种(design_log 十四);`paper_v01r/`:v0.1 + 监管事件覆盖层(design_log 13.6/13.7,事件由每日 params 推导,大商所缺);`paper_v05/`:v0.3 信号、只做 12 个工业品(design_log 十五,事后假设的前向检验)。同一数据、同一成交规则;每日脚本先跑主账本(拉数据),再跑另两本(不拉)。
